  ...
  var confirmTempUser = function(url, callback) {
    ...
    TempUser.findOne(query, function(err, tempUserData) {
      ...
      if (tempUserData) {
        ...
        // check for existing link to real user
        if (tempUserData.userId !== null) {
          return options.persistentUserModel.findOne({ _id: tempUserData.userId }, function(err, user) {
            if (err) {
              return callback(err, null);
            }
        ...
        user.save(function(err, savedUser) {
          ...
          // update the temporary user with the new user's id
          TempUser.findOneAndUpdate(query, { '$set': { userId: savedUser._id } }, function(err, updatedTempUser) {
            if (err) {
              return callback(err, null);
            }
